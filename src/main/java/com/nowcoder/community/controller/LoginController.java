...
public class LoginController implements CommunityConstant {
    ...
    /*
    * 如果有错误的信息，希望回到login页面展示，此时需要保留原来的username和password,
    * 如果参数是一个实体，springmvc会把实体放到model里，页面可以直接在model中获取
    * 如果不是一个实体是普通参数类型，可以人为加到model中，也可以在request中获取
    * */
    @RequestMapping(path = "/login", method = RequestMethod.POST) // 两个方法的路径可以一样，但前提是请求方式要有所区别
    public String login(String username, String password, String code,
        ...
        if(map.containsKey("ticket")){
            ...
            cookie.setPath(contextPath);   //设置cookie的有效时间
            cookie.setMaxAge(expiredSeconds); //设置cookie的超时时间
            response.addCookie(cookie); //把cookie放入response中
